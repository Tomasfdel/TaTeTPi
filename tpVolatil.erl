...
pbalance(Dictionary) -> receive {update, Node, Workload} -> pbalance(dict:store(Node, Workload, Dictionary));
                                {getmin, Node} -> io:format("El diccionario es: ~p~n", [Dictionary]),
                                                  io:format("Llamo al fold~n"),
                                                  FoldResult = dict:fold(minWorkloadNode/3, {dummyNode, ?Infinite}, Dictionary),
                                                  io:format("El resultado del fold es: ~p~n", [FoldResult]),
                                                  Node ! {balanceResponse, element(1, FoldResult)},
...
psocket(Socket, Username) -> 
	receive {tcp, Socket, Msg} -> 	whereis(balance) ! {getmin, node()},
	                                receive {balanceResponse, SpawningNode} -> spawn(SpawningNode, pcomando, [self(), Msg, Username]), 
